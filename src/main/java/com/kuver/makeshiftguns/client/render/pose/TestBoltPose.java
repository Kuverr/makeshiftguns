...
public class TestBoltPose extends WeaponPose {
    public TestBoltPose() {
    }
...
//        poseStack.translate((double) reloadProgress * 0.5, (double) (-reloadProgress), (double) (-reloadProgress) * 0.5);
        ...
        poseStack.translate(0.3 * (double) side, 0, 0.1);
        ...
        //due to the arm being rotated beforehand Z and Y are swapped

        poseStack.translate(-0.2 * cooldown, 0, 0);
        if (cooldown > 2F) {
            poseStack.translate(0, -0.5 * cooldown, 0);
        }

        poseStack.mulPose(Vector3f.XP.rotationDegrees(-15F * cooldown));
//        poseStack.translate(0, (double) (-cooldown) * 0.2, 0);
